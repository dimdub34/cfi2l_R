...
1. [Présentation et installation de R](#presentation)
2. [Présentation de RStudio](#rstudio)
3. [Structures de données](#structures)
4. [Préparation d'un jeu de données](#preparation)
5. [Manipulation d'un jeu de données](#manipulation)
6. [Analyse d'un jeu de données](#analyse)
7. [Visualisation d'un jeu de données](#visualisation)
  

# 1. Présentation de R {#presentation}
...
- pour stocker une valeur il faut créer une variable
...
- on affecte la valeur 10 à la variable x
...
- les variables apparaissent dans l'onglet "Environnement", ce sont les objets en mémoire pour la session en cours
...
- permet définir le dossier dans lequel tous les fichiers du projet seront stockés (sources et sorties)
- évite les chemins absolus, utilisation des **chemins relatifs**
- pour créer un nouveau projet : File/New Project
- soit créer un nouveau dossier sur l'ordinateur, soit utiliser un dossier existant
...
- la console ne conserve pas le code une fois la session fermée
...
- c'est un fichier texte, enregistré avec l'extension `*.R`
...
- mélange de Markdown (html simplifié) et de code (avec la sortie juste en dessous)
...
A chaque enregistrement du notebook, R génère le fichier html du notebook
...
- extensions des fonctionnalités de base (R Core)
- développés et maintenus par la communauté
- regroupés au sein du CRAN (Comprehensive R Archive Network)
...
- numériques : integer (entier) ou double (décimaux)
...
💡 *Dans un chunk, mettre des parenthèses autour du code exécute le code et affiche le résutat*

---

Un vecteur est un **objet indexé**, c'est à dire que chaque élément a un numéro d'ordre, appelé index, qui permet de le retrouver.  
Pour accéder à un élément du vecteur, utilisation de l'opérateur \[\] avec dedans l'index de l'élément souhaité  
⚠️ **la numérotation commence à 1**
...
**plusieurs éléments via un vecteur d'index**
...
Une opération sur un vecteur s'applique à **tous les éléments du vecteur**
...
**Autres méthodes pour créer un vecteur**  

Avec numeric(), character(), logical() pour des vecteurs vides.  
**Avec un entier en argument cela crée le vecteur avec le nombre d'éléments correspondants**, avec des valeurs par défaut (0 pour numeric, "" pour character et FALSE pour logical)

```{r}
numeric(10)
```

**Avec l'opérateur '`:`'** : crée un vecteur d'entiers qui se suivent
...
Avec la fonction **`seq`** qui permet de créer une séquence de nombres

```{r}
seq(from=1, to=10, by=2)
```

Avec la fonction **`rep`** qui permet de répéter un vecteur
...
- Il existe des fonctions pour générer des vecteurs de données aléatoires : `rnorm`, `runif`, `rbinom`, `rpois` etc.

---

💻 **Pratique**

A partir des données ci-dessous (4 foyers), calculer le revenu global du foyer puis le revenu par personne.
...
💡 *Il existe aussi rowSums et colSums pour les sommes*

--- 

💻 **Pratique**

1. A partir des deux vecteurs ci-dessous, calculer la témpérature moyenne et le total des précipitations
...
2. Avec les vecteurs de notes suivants (6 élèves) :
...
- Charger le jeu de données `hdv2003` du package `questionr`
- Appeler chacune des fonctions de "découverte" sur le jeu de données `hdv2003`.


## La matrice

- tableau de données bi-dimensionnel (lignes, colonnes)
- chaque élément est de même type
- utilisée pour les calculs matriciels (calculs mathématiques et opérations algébriques)

**Création**

- `matrix(data, nrow, ncol, byrow=FALSE, dimnames=NULL)`
- `diag(n)` : matrice diagonale de taille n
- `diag(v)` : matrice diagonale avec les éléments de v sur la diagonale

```{r}
# matrice 3x3 avec des nombres de 1 à 9
...
- `det(mat)` : déterminant de la matrice

## La liste

Structure de données qui peut contenir des éléments de types différents (vecteur, matrice, dataframe, autre liste, fonction)  
...
- la première étape de traitement d'un jeu de données comprend le nettoyage, le recodage et la création de nouvelles variables si besoin
...
Ensemble de librairies créées pour faciliter le chargement, le nettoyage, le tri, l'analyse de données et les représentations graphiques  
`library(tidyverse)`
...
- les fonctions des librairies du tidyverse acceptent un dataframe en entrée, mais retournent un tibble
...
un tibble peut également être converti en dataframe avec la fonction as.data.frame()

## Lecture d'un jeu de données

- le csv est le format "standard" pour le stockage et le partage de données 
- c'est un fichier texte avec un séparateur de colonnes (le point-virgule, la virgule ou la tabulation)
- deux fonctions pour ouvrir un fichier csv:
  + read_csv : fichiers avec une virgule comme séparateur et le point en décimale
  + read_csv2 : fichiers avec le point-virgule comme séparateur et la virgule en décimale (comme en France par exemple)

💡 *RStudio propose une interface graphique pour l'import de données, en cliquant sur le bouton "Import Dataset" dans l'onglet Environment*

## Ecriture d'un jeu de données

Pour écrire dans un fichier csv, il y a les fonctions write_csv et write_csv2
...
💡 *la fonction `freq`, du package `questionr` fournit des informations sur une variable catégorielle : effectifs, pourcentages (avec et sans NA)*  
*`?questionr::freq` pour afficher l'aide*

---

💡 *Les modalités d'une variable catégorielle sont appelé des `levels`*
...
```
💡 le "$" permet d'afficher un vecteur d'un dataframe, mais permet aussi d'en créer un nouveau, autrement dit de créer une nouvelle variable dans le dataframe. 

---

Pour recoder une modalité en variable manquante utilisation de `NULL` comme nouveau nom
...
Pour coder les `NA` en modalité, fonction `fct_na_value_to_level`
...
1. `fct_other()` pour regrouper un ensemble de modalités dans la catégorie "Other"
2. `fct_lump()` pour regrouper les modalités les moins fréquentes dans la catégorie "Other"
3. interface graphique, proposée par `questionr`, à partir de **`Addins`** puis **`Levels Recoding`**.
...
1. dans les données hdv2003, recoder la variable `relig` afin d'obtenir
```
                              n    % val%
...
3. ré-ordonner les modalités de cette variable afin d'obtenir
```
                                         n    % val%
Enseignement superieur                  441 22.0 23.4
Enseignement technique ou professionnel 594 29.7 31.5
...
*à partir d'une variable numérique*  

**`cut()`**

- création de classes à partir d'une variable numérique
- exemple : une variable de revenu
- l'argument `breaks` va permettre de définir les intervalles des classes

`breaks=nombre` : l'interpréteur va créer `nombre` classes de même largeur
...
`breaks=vecteur` : l'interpréteur va créer des classes en fonction des valeurs du vecteur
...
```
💡 le package *questionr* propose une interface graphique : Addins/Numeric range dividing

---

**`if_else`**

Créer une variable catégorielle à deux modalités en fonction des valeurs d'une variable numérique
...
`case_when` : étend `if_else` à plusieurs conditions, sous la forme `condition ~ valeur`   
⚠️ les tests sont effectués dans l'ordre, donc il faut aller du plus spécifique au plus large
...
1. découper la variable heures.tv en classes afin d'obtenir le tableau de fréquences suivant
...
2. avec `if_else`, créer la variable `cinema_bd` qui groupe les personnes qui vont au cinéma et qui lisent des bd. Les autres sont mis dans "Autre". Le résultat attendu est
```
               n    % val%
...
3. avec `case_when` créer la variable `genre_fratrie` ayant les modalités : Homme avec plus de 2 frères et soeurs, Femme avec plus de 2 frères et soeurs, Autre. Le résulat attendu est
```
                                         n    % val%
...
les colonnes 2002 et 2007 devraient être dans une colonne "année" et les valeurs dans une colonne "population"
...
Pour les exemples, utilisation des datasets du package `nycflights23` (données des vols de départ de 3 aéroports de New-York en 2023)  
...
Tous les vols qui ont entre 10 et 15 minutes de retard au décollage
...
1. `dplyr` combine automatiquement avec l'opérateur `&` les conditions séparées par une virgule, donc `filter(flights, dep_delay >= 10, dep_delay <= 15)` est équivalent à `filter(flights, dep_delay >= 10 & dep_delay <= 15)`.
2. pour le `OU` par contre, il faut forcément `filter(flights, dep_delay < 10 | dep_delay > 15)`
...
Sélection des colonnes latitude et longitude
...
`select` peut prendre comme argument des éléments permettant une sélection selon des critères
...
Fonction `desc()` pour inverser l'ordre (décroissant)
...
```
Création d'une variable `air_time_hours` 
...
Les actions vues précédemment peuvent être enchaînées grâce au `pipe` c'est à dire l'envoi du résultat de l'action précédente comme premier argument de l'action suivante.  
C'est l'opérateur `|>` qui joue ce rôle
...
⚠️ Attention les actions s'enchaînent (pipeline) mais rien n'est conservé. 
Pour enregistrer le résultat dans un objet il faut commencer par une affectation (`<-`)
...
Le résultat du pipeline sera stocké dans l'objet delays.

---

💻 **Pratique**

1. afficher les lignes 100 à 105 du dataset flights
2. afficher les vols du mois de juillet 2023
3. afficher les vols qui ont eu un retard (variable arr_delay) compris entre 5 et 15 minutes. Le résultat doit être affiché par ordre décroissant
4. selectionner les colonnes name, lat et lon du dataset airports et renommer lat et lon en latitude et longitude
5. dans le dataset airports, créer une nouvelle variable alt_metres qui contient l'altitude en mètres (conversion de pieds en mètres en divisant la variable alt par 3.28)

## group_by

Permet de grouper les données puis d'appliquer une fonction à ces données groupées  

Grouper les données par mois et affiche le premier index (combinaison group_by et slice)
...
Grouper les données par compagnie, calculer le delay moyen et le mettre dans une variable mean_delay (combinaison group_by et mutate)
...
Permet d'agréger une information
...
Utilisé avec `group_by`, `summarise` permet d'agréger une information sur des données groupées  

Le retard moyen de chaque compagnie et ordonné du plus grand retard moyen au plus petit
...
L'argument `n=n()` dans `summarise` donne le nombre d'observations utilisées pour le calcul
...
Utilisation d'une donnée créée par `summarise` pour créer une nouvelle variable avec `mutate`
...
1. afficher le nombre de vols par mois et afficher le résultat par ordre croissant du nombre de vols
2. calculer le nombre de vols à destination de Los Angeles (code LAX) pour chaque mois de l'année
3. calculer le nombre de vols par mois et par destination et afficher uniquement le max
4. calculer le nombre de vols par mois et créer une colonne qui affiche le pourcentage que cela représente sur les vols annuels
...
- dépend de son type: 
  + quantitative (comme l'âge ou le revenu par exemple)
  + qualitative / catégorielle (nombre limité de valeurs possibles, comme le genre ou la profession par exemple)
...
Pour des pourcentages, la fonction `prop.table()` avec en argument l'objet renvoyé par `table()`
...
- la dernière colonne est sans comptabiliser les NA
...
- étudier la relation entre 2 variables
- l'analyse va dépendre du type des variables: 
...
Tableau croisé, avec la fonction `table()` en lui passant deux vecteurs
...
`questionr` propose `lprop` et `cprop`, avec en plus le total et un formatage par défaut d'une décimale
...
- test d'indépendance du $\chi^2$ pour savoir si la répartition homme/femme dans chaque métier est proche ou non. 
- hypothèse $H_0$ : la profession et le genre sont deux variables indépendantes

```{r}
chisq.test(tb_prof_genre)
```

- `X-squared` est la valeur de la statistique du $\chi^2$, c'est une mesure de distance entre le tableau donné et le tableau théorique si les deux variables étaient indépendantes
- `df` est le nombre de degrés de liberté (ddl=(nombre de lignes - 1) x (nombre de colonnes  -1))
...
Déterminer si les valeurs de la variable quantitative se répartissent différemment ou non selon les modalités de la variable qualitative
...
- tester si l'âge moyen est statistiquement différent entre les sportifs et les non-sportifs 
- si la variable quantitative suit un loi normale test paramétrique de student, sinon test non-paramétrique de Mann-Whitney
...
Test de Mann-Whitney : wilcox.test avec argument paired=F car les deux échantillons (sportifs et non-sportifs) sont indépendants (ce ne sont pas les mêmes personnes)

```{r}
wilcox.test(hdv2003$age ~ hdv2003$sport, paired=F)
```

Test de Student ($H_0$: l'âge moyen des deux modalités n'est pas différent)

```{r}
t.test(hdv2003$age ~ hdv2003$sport)
```

dans les deux tests la p-value est inférieure à 5% donc $H_0$ est rejetée, autrement dit l'âge moyen des sportifs est significativement inférieur à celui des non-sportifs
...
Pour déterminer la relation entre deux variables numériques, calcul du coefficient de corrélation.  
Le coefficient de corrélation varie entre -1 et 1, il mesure la force et le sens de la relation entre deux variables.  
Un coefficient proche de -1 indique une relation négative (lorsque l'une des variables augmente l'autre diminue), et un coefficient proche de 1 indique une relation positive (les deux variables vont dans le même sens).

Si la relation est linéaire utilisation du coefficient de corrélation de Pearson (méthode par défaut)
...
Rejet de $H_0$, la corrélation est significativement différente de 0.

---

Si la relation n'est pas linéaire il faut utiliser la méthode des rangs de Spearman
...
Si la relation entre deux variables est linéraire, estimation de la regression linéaire, c'est à dire des paramètres de la droite qui représente au mieux le nuage de points.

```{r}
lm(rp2012$cadres ~ rp2012$dipl_sup)
```

La constante est égale à 0.9217 et la pente de la droite à 1.0816

---

Si la sortie de la fonction `lm` est placée en argument de la fonction `summary` alors davantage d'informations
...
dataset pour travailler : un sous-ensemble des données rp2012
...
**Fonctionnement général de ggplot2**
...
- la fonction `aes()` permet de mapper les variables du dataset aux éléments du graphique
...
Exemple: calcul du pourcentage que représente le nombre de communes de chaque département par rapport au total
...
rp |> 
  ...
  ggplot() + 
  geom_bar(aes(x=departement, y=pourcent), stat="identity")
```
...
**`geom_bar` avec coordonnées polaires**  
ggplot2 ne propose pas de graphique camembert, mais on peut le faire avec coordonnées polaires  
...
rp |> 
  ...
  mutate(pourcent=n/sum(n) * 100) |>
  ggplot() + 
  geom_bar(aes(x="", y=pourcent, fill=departement), stat="identity") + 
  coord_polar("y")
```
...
Personnalisation du graphique avec paramètres supplémentaires dans `geom_point`
...
**Ajout d'une droite de régression avec `geom_smooth`** en plus de `geom_point`

💡 Si plusieurs `geom` utilisent les mêmes éléments dans `aes`, la fonction `aes()` peut être appelée dans `ggplot`
...
💡 Pour ajouter plusieurs courbes dans le graphique, plusieurs appels à `geom_line` (avec "+" entre chaque)
...
- Permet de visualiser une troisième dimension (voire une quatrième)
...
Ajout d'une 4ème dimension avec la taille des points, ici la population totale de la commune
...
Décomposer un graphique en plusieurs, selon les modalités d'une ou plusieurs variables catégorielles
...
`scale_x_discrete` et `scale_y_discrete` pour les variables catégorielles
...
💡 le package `esquisse` propose un Addins pour faire les graphiques `Addins/ggplot2 builder`
...
2. même graphique que précédemment mais avec un mappage de couleur selon la région
3. boxplot: distribution de la variable proprio par rapport à la taille de departement (pop_cl)
4. nuage de points avec en x=dipl_sup et y=cadres avec un graphique par région (facet_wrap)
5. boxplot: distribution du pourcentage moyen de propriétaires par département au sein de chaque région
