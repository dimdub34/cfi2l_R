...
- si la variable qualitative est binaire (2 modalités), test de Mann-Withney (non paramétrique) ou test de student (paramétrique)
- si la variable qualitative a plus de 2 modalités, test de Kruskal-Wallis (non paramétrique) ou test de Anova (paramétrique)

Paramétrique ou non : test de normalité de la variable quantitative, avec le test de Shapiro - $H_0$ la distribution suit une loi normale.  
Si la variable quantitative suit un loi normale test paramétrique, autrement test non paramétrique.
...
**`geom_col`** pour un graphique en colonnes avec des valeurs personnalisées (ici le pourcentage)
...
**`geom_col` avec coordonnées polaires**  
...
```{r, eval=F}
data(package="ggplot2", "economics")
```
...
- `theme` pour l'aspect général
- `labs` pour les titres des axes et du graphique
- `scale_*` pour la personnalisation des échelles

---

`theme` pour la personnalisation

Existe : `theme_gray` (défault), `theme_minimal`, `theme_light`, `theme_dark`, `theme_bw`, `theme_classic`, `theme_void`
...
`labs` pour les titres

```{r, fig.height=4}
ggplot(rp) +
  geom_point(aes(x=dipl_sup, y=cadres)) +
  labs(title="Relation entre diplômes supérieurs et cadres", x="Diplômes supérieurs", y="Cadres")
```
...
4. graphique en barres avec le nombre total de propriétaires par région
5. nuage de points avec en x=dipl_sup et y=cadres avec un graphique par région (facet_wrap)
