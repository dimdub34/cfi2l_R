...
- les variables appraîssent dans l'onglet "Environnement", ce sont les objets en mémoire pour la session en cours
...
- elles sont regroupées au sein du réseau CRAN (Comprehensive R Archive Network)
...
💡 *Dans un chunk, des parenthèses autour du code exécute le code et affiche le résutat*
...
**Combinaison de données avec `rbind` et `cbind`**  
...
**Calcul de moyennes avec `rowMeans` et `colMeans`**
...
Ensemble de librairies avec pour objectif de faciliter le chargement, le nettoyage, le tri, l'analyse de données et les représentations graphiques  
`library(tidyverse)`
...
**Dans le tidyverse, le `tibble` est la version moderne du dataframe**
...
Un "`-`" devant le nom de colonne cela affiche toutes les colonnes sauf elle
...
- l'analyse va du type des variables: 
...
Ajout d'une 4ème dimension avec la taille des points, qui représente la population totale de la commune
