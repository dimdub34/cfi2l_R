...
- Integrated Development Environment (IDE ou *Environnement de Developpement Intégré*) pour R
...
- en R tout est objet donc une variable est un objet
- dès que l'on crée un objet il apparaît dans l'onglet "Environnement"
- les commandes entrées dans la console apparaissent dans l'onglet "History"

##

- on peut créer autant d'objet que l'on souhaite et faire des opérations avec ces objets
```{r}
y <- 100
x + y
```
- les noms des objets peuvent contenir des lettres, chiffres et les symboles "." et "_"
- ne peuvent pas commencer par un chiffre
- ne pas mettre d'accents
- sensible à la casse (x et X sont deux objets différents)
...
- html simplifié
- utilisé pour structurer le Notebook avec des titres, des morceaux de texte en gras ou italique, des liens, des listes à puces ou numérotées etc.
...
- créer un nouveau projet (création d'un nouveau dossier R dans P:/)
...
- pour stocker plusieurs valeurs dans le même objet
...
- possible créer des vecteurs vides, en précisant le type: numeric(), character(), logical()
- si on ajoute un entier en argument cela crée le vecteur avec le nombre d'éléments correspondants, avec des valeurs par défaut (0 pour numeric, "" pour character et FALSE pour logical)
...
(x <- c(120, 134, 256, 12))
```
utiliser ce vecteur pour générer les deux vecteurs suivants
```{r, echo=F}
x + 100
```
```{r, echo=F}
...
```
- calculer le revenu global du foyer, puis le revenu par personne

## Fonctions

- pour effectuer des calculs ou accomplir des actions
- une fonction est un objet qui contient un ensemble d'instructions et renvoie un résultat
- on appelle une fonction avec son nom suivi de parenthèses avec dedans zéro, un ou plusieurs paramètres (arguments)
- appel ci-dessous de la fonction *mean* qui calcule la moyenne des valeurs du vecteur passé en paramètre
```{r}
mean(tailles)
```
```{r}
...
- calculer le total des précipitations

soit les vecteurs de notes suivants (6 élèves):
```{r, eval=F}
...
##
chercher dans l'aide pour comprendre le vecteur renvoyé par la fonction pmin (et pmax)
```{r, echo=F}
python <- c(12, 16, 8, 18, 6, 10)
html <- c(14, 9, 13, 15, 17, 11)
bdd <- c(18, 11, 14, 10, 8, 12)
...
## 
### Fonctions utiles quand on découvre un jeu de données
